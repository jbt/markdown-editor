...
function sanitize(html) {
  ...
  return html
    .split("```")
    .map((subHTML, index) => {
      if (!isOdd(index)) return subHTML;

      let old;
      do {
        old = subHTML;
        subHTML = subHTML.replace("&lt;", "<").replace("&gt;", "/>");
      } while (subHTML !== old);
...
function selectionChanger(selection, operator, endoperator) {
  ...
  var isApplied =
    selection.slice(0, 2) === operator && selection.slice(-2) === endoperator;
  var finaltext = isApplied
    ? selection.slice(2, -2)
    : operator + selection + endoperator;
  return finaltext;
}
...
document
  .getElementById("saveas-markdown")
...
function start() {
  ...
  if (window.location.hash) {
    ...
    if (h.slice(0, 5) == "view:") {
      setOutput(
        decodeURIComponent(escape(RawDeflate.inflate(atob(h.slice(5)))))
      );
...
window.addEventListener("beforeunload", function (e) {
  if (
    !editor.getValue() ||
    editor.getValue() == localStorage.getItem("content")
  ) {
    return;
  }
